import { type IncludeDeleted, type UserGroup, type UserGroupAdminView } from 'src/global_types'
...
export async function createUserGroup(i: { name: string; userSlugs: string[] }): Promise<void> {
  let slug = i.name
    .toLowerCase()
    .replace(/[^A-Za-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
