...
export async function getEvidenceMigrationDifference(i: {
  ...
  return await ds.getEvidenceMigrationDifference(
    { operationSlug: i.toOperationSlug, evidenceUuid: i.evidenceUuid },
    i.fromOperationSlug,
  )
}
...
export async function updateEvidence(i: {
  ...
  if (i.oldTags && i.newTags) {
    const [adds, subs] = computeDelta(
      i.oldTags.map((tag) => tag.id),
      i.newTags.map((tag) => tag.id),
    )
...
export async function changeFindingsOfEvidence(i: {
  ...
  const [adds, subs] = computeDelta(
    i.oldFindings.map((f) => f.uuid),
    i.newFindings.map((f) => f.uuid),
  )

  await Promise.all(
    adds.map((findingUuid) =>
      ds.updateFindingEvidence(
