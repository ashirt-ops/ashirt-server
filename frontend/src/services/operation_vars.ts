...
export async function createOperationVar(
  ...
  try {
    ...
    if (
      ...
      err.message.match(/An operation variable with this name already exists/g)
    ) {
      slug += '-' + Date.now()
...
export async function updateOperationVar(
  operationSlug: string,
  varSlug: string,
  i: { value: string | null; name: string | null },
): Promise<void> {
