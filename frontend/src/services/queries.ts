...
export async function getSavedQueries(i: { operationSlug: string }): Promise<Array<SavedQuery>> {
...
export async function updateSavedQuery(i: {
  ...
  await ds.updateQuery(
    { operationSlug: i.operationSlug, queryId: i.queryId },
    {
      name: i.name.trim(),
      query: i.query.trim(),
    },
