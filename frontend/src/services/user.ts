...
export async function createRecoveryCode(i: { userSlug: string }): Promise<string> {
  const resp = await ds.createRecoveryCode(i)
  return resp.code
}

export async function getTotpForUser(i: { userSlug: string }): Promise<boolean> {
  return await ds.getTotpForUser(i)
}

export async function deleteTotpForUser(i: { userSlug: string }): Promise<void> {
