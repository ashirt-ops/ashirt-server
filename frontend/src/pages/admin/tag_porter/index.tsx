  ...
  return (
    <>
      <SettingsSection title="Import/Export Default Tags">
        <em>
          You can share your default tags with other AShirt users by import or exporting the default
          tags here.
        </em>
        {wiredTags.render((tags) => (
          <>
            <div className={cx('button-group')}>
              <Button primary onClick={() => importTagsModal.show({})}>
                Import
              </Button>
              <Button
  ...
  const formComponentProps = useForm({
    ...
    handleSubmit: async () => {
      ...
      try {
        ...
        if (isError(err)) {
          throw new Error(
            err instanceof SyntaxError ? 'Unable to parse data / Unsupported format' : err.message,
          )
        } else {
...
const TagImportUpload = (props: { contentFilename: string | null; err: Error | null }) => {
  ...
  return <div className={cx('has-content')}>Selected File: "{props.contentFilename}"</div>
}
