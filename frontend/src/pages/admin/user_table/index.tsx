...
export default function UserTable(props: {
  ...
  const recoverFn = (u: UserAdminView) =>
    createRecoveryCode({ userSlug: u.slug }).then(setRecoveryCode)
  const actionsBuilder = actionsForUserBuilder(
    self ? self.slug : '',
    editUserFn,
    ...
    <SettingsSection title="User List" width="wide">
      ...
      <Table className={cx('table')} columns={columns}>
        {wiredUsers.render((data) => (
          <>
            {data.map((user) => (
              <TableRow key={user.slug} data={rowBuilder(user, actionsBuilder(user))} />
            ))}
          </>
        ))}
      </Table>
      ...
      {resettingPassword && (
        <ResetPasswordModal
          ...
          onRequestClose={() => setResettingPassword(null)}
        />
      )}
      {editingUserFlags && (
        <UpdateUserFlagsModal
          user={editingUserFlags}
          onRequestClose={() => {
            setEditingUserFlags(null)
            wiredUsers.reload()
          }}
        />
      )}
      ...
      {recoveryCode && (
        <RecoverAccountModal
          ...
          onRequestClose={() => setRecoveryCode(null)}
        />
      )}
    </SettingsSection>
  )
  ...
  (u: UserAdminView) => {
    ...
    const notDeletedOrSelf = (msg?: string) => {
      switch (true) {
        case u.deleted:
          return deletedAttrs
        case u.slug === selfSlug:
          return { disabled: true, title: msg }
        default:
          return {}
      }
    }
    ...
    return (
      <ButtonGroup>
        <ClickPopover
          ...
          content={
            <Menu>
              ...
              <MenuItem onClick={() => resetPwFn(u)} {...canReset()}>
                Reset Password
              </MenuItem>
              <MenuItem onClick={() => editFlagsFn(u)} {...canEditFlags}>
                Edit User Flags
              </MenuItem>
              <MenuItem onClick={() => deleteTotpFn(u)} {...canRemoveTotp()}>
                Remove Multi-Factor Authentication
              </MenuItem>
              <MenuItem onClick={() => recoveryFn(u)} {...canRecover}>
        ...
        <Button danger small onClick={() => deleteUserFn(u)} {...canDelete}>
          Delete User
        </Button>
      </ButtonGroup>
    )
