...
export const ResetPasswordModal = (props: { user: User; onRequestClose: () => void }) => {
...
export const AddUserModal = (props: { onRequestClose: () => void }) => {
    ...
    handleSubmit: () => {
      if (firstName.value.length == 0) {
        return new Promise((_resolve, reject) =>
          reject(Error('Users should have at least a first name')),
        )
      }
      ...
      {isDisabled && (
        <>
          <div className={cx('success-area')}>
            ...
            <Button className={cx('success-close-button')} primary onClick={props.onRequestClose}>
              Close
            </Button>
          </div>
        </>
    ...
    <Modal title="Create New Group" onRequestClose={props.onRequestClose}>
      {isCompleted ? (
        ...
        <>
          ...
          <SimpleUserTable
            {...bus}
            setIncludedUsers={setIncludedUsers}
            includedUsers={includedUsers as Set<string>}
          />
          <Form
            {...formComponentProps}
            loading={isCompleted}
          ...
          >
            <h1 className={cx('header')}>
              Name<span className={cx('optional')}>*</span>
            </h1>
...
export const ModifyUserGroupModal = (props: {
  ...
  const formComponentProps = useForm({
    ...
    handleSubmit: () => {
      ...
      const nameOrNull =
        name.value.toLowerCase() !== props.userGroup.name.toLowerCase() ? name.value : null
      const runSubmit = async () => {
...
export const UpdateUserFlagsModal = (props: {
  ...
  const canAlterAdmin = () => {
    if (adminIsTargetUser) {
      return badAdmin
    }
    if (isHeadlessUser) {
      return { disabled: true, title: 'Headless users cannot be admins' }
    }
    return {}
  }
...
export const DeleteUserModal = (props: { user: UserAdminView; onRequestClose: () => void }) => (
  <ChallengeModalForm
    modalTitle="Delete User"
...
export const DeleteGlobalAuthSchemeModal = (props: {
  ...
  <ChallengeModalForm
    ...
    warningText={`This will unlink/remove this authentication scheme from all users.${
      props.uniqueUsers == 0
        ? ''
        : ` Note that this will effectively disable ${props.uniqueUsers} accounts.`
    }`}
...
export const RecoverAccountModal = (props: {
  ...
  return (
    <Modal title="Recovery URL" onRequestClose={props.onRequestClose}>
      <div className={cx('recovery-code-modal')}>
        <p>
          Below is the recovery URL. Provide this to the user, and they will be able to log in
...
export const InviteUserModal = (props: { onRequestClose: () => void }) => {
  const firstName = useFormField<string>('')
  const lastName = useFormField<string>('')
  const contactEmail = useFormField<string>('')
...
export const RemoveTotpModal = (props: { user: UserAdminView; onRequestClose: () => void }) => {
  const formComponentProps = useForm({
    fields: [],
    onSuccess: () => props.onRequestClose(),
  ...
  return (
    ...
    >
      <em className={cx('warning')}>
        Multi-factor Authentication provides an extra layer of security for this user. Removing this
        factor should only be done if the user has lost the device or the mechansim to authenticate.
      ...
      {isCompleted ? (
        ...
        <>
          ...
          >
            ...
            <Input label="" {...name} disabled={isCompleted} />
            <h1 className={cx('header')}>
              Value<span className={cx('optional')}>*</span>
            </h1>
...
export const DeleteVarModal = (props: {
  ...
  <ChallengeModalForm
    ...
    handleSubmit={() =>
      isOperationVariable(props.variableData)
...
export const ModifyVarModal = (props: {
  ...
  const formComponentProps = useForm({
    ...
    handleSubmit: () => {
      if (name.value.length == 0) {
        return new Promise((_resolve, reject) =>
          reject(Error('Global Variable should have a name')),
        )
      }

      const nameOrNull =
        name.value.toLowerCase() !== props.variableData.variable.name.toLowerCase()
          ? name.value
          : null
      const valOrNull =
        value.value.toLowerCase() !== props.variableData.variable.value.toLowerCase()
          ? value.value
          : null
      ...
      const runSubmit = async () => {
        if (copiedValue) {
          ...
          const update = isOperationVariable(props.variableData)
            ? await updateOperationVar(
                props.variableData.operationSlug,
                props.variableData.variable.name,
                {
                  value: valOrNull,
                  name: nameOrNull,
                },
