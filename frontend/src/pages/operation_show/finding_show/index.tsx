...
import {
  ChangeEvidenceOfFindingModal,
  RemoveEvidenceFromFindingModal,
  EditFindingModal,
  DeleteFindingModal,
} from '../finding_modals'
...
export default () => {
  ...
  const addRemoveEvidenceModal = useModal<{ finding: Finding; initialEvidence: Array<Evidence> }>(
    (modalProps) => (
      <ChangeEvidenceOfFindingModal
        {...modalProps}
        onChanged={reloadToTop}
  ...
  const deleteFindingModal = useModal<{ finding: Finding }>((modalProps) => (
    <DeleteFindingModal
      {...modalProps}
      onDeleted={() => navigate(`/operations/${operationSlug}/findings`)}
      operationSlug={operationSlug}
    />
  ))
  ...
  const removeEvidenceFromFindingModal = useModal<{ evidence: Evidence; finding: Finding }>(
    (modalProps) => (
      <RemoveEvidenceFromFindingModal
        {...modalProps}
        onRemoved={reloadToTop}
        operationSlug={operationSlug}
      />
    ),
  )
        ...
        <div className={cx('root')}>
            ...
            <div className={cx('actions')}>
              ...
              <ButtonGroup className={cx('right')}>
                <Button
                  small
                  onClick={() =>
                    addRemoveEvidenceModal.show({ finding, initialEvidence: evidence })
                  }
                >
                  Add/Remove Evidence
                </Button>
                <Button small onClick={() => editFindingModal.show({ finding })}>
                  Edit
                </Button>
                <Button small onClick={() => deleteFindingModal.show({ finding })}>
                  Delete
                </Button>
              </ButtonGroup>
            </div>
          ...
          <div className={cx('timeline')}>
            <Timeline
              ...
              onQueryUpdate={(query) =>
                navigate(
                  `/operations/${operationSlug}/evidence?q=${encodeURIComponent(query.trim())}`,
                )
              }
              operationSlug={operationSlug}
              query=""
