...
export default (props: {
  ...
  <Table
    className={cx('table')}
    columns={['Title', 'Category', 'Ticket', '# Evidence', 'Date Range', 'Tags']}
  >
    {props.findings.map((finding) => (
      <tr key={finding.uuid}>
        <td className={cx('title-cell')}>
          <Link
            to={`/operations/${props.operationSlug}/findings/${finding.uuid}`}
            className={cx('title')}
          >
            {finding.title}
          </Link>
          <ButtonGroup className={cx('actions')}>
            <Button small onClick={() => props.onEdit(finding)}>
              Edit
            </Button>
            <Button small onClick={() => props.onDelete(finding)}>
              Delete
            </Button>
          </ButtonGroup>
        </td>
        ...
        <td>
          {finding.occurredFrom && finding.occurredTo
