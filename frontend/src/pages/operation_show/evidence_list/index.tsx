...
export default () => {
  ...
  const viewModal = useModal<{ evidence: Evidence }>((modalProps) => (
    <EvidenceMetadataModal
      {...modalProps}
      operationSlug={operationSlug}
      onUpdated={wiredEvidence.reload}
    />
  ))
        ...
        if (e.contentType === 'codeblock') {
          ...
          return {
            ...
            blob: new Blob([data.content], {
              type: `text/${contentToFileExtension[e.contentType][data.contentSubtype]}`,
            }),
          }
        } else {
  ...
  const exportEvidence = async () => {
    ...
    if (media) {
      media.forEach((mb) => {
        ...
        if (
          mb.contentType === 'codeblock' &&
          mb.contentSubtype === 'c_cpp' &&
          fileExtension === 'cpp'
        ) {
