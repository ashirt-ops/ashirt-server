      ...
      <SettingsSection title="Operation Variables" width="wide">
        <Table columns={columns}>
          {wiredOperationVars.render((data) => (
            <>
              {data?.map((operationVar) => (
                <TableRow
                  key={operationVar.name}
                  operationVar={operationVar}
                  data={rowBuilder(
                    operationVar,
                    modifyActions(operationVar, setDeletingOperationVar, setModifyingOperationVar),
                  )}
                />
              ))}
            </>
        ...
        {deletingOperationVar && (
          <DeleteVarModal
            variableData={{ variable: deletingOperationVar, operationSlug: props.operationSlug }}
            onRequestClose={() => {
              setDeletingOperationVar(null)
              wiredOperationVars.reload()
            }}
          />
        )}
        {modifyingOperationVar && (
          <ModifyVarModal
            variableData={{ variable: modifyingOperationVar, operationSlug: props.operationSlug }}
            onRequestClose={() => {
              setModifyingOperationVar(null)
              wiredOperationVars.reload()
            }}
          />
        )}
...
const TableRow = (props: { data: Rowdata; operationVar: OperationVar }) => (
  <tr>
...
const modifyActions = (
  ...
  return (
    <ButtonGroup>
      <Button small onClick={() => onEditClick(u)}>
        Edit
      </Button>
      <Button small danger={true} onClick={() => onDeleteClick(u)}>
        Delete
      </Button>
    </ButtonGroup>
  )
