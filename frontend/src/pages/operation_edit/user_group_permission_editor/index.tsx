...
const NewUserGroupForm = (props: { operationSlug: string; requestReload: () => void }) => {
...
const PermissionTableRow = (props: {
  ...
  return (
    <>
      <tr>
        <td style={{ fontWeight: isCurrentUser ? 800 : 400 }}>{props.userGroup.name}</td>
        <td>
          <RoleSelect
            disabled={disabled}
            value={props.role}
...
const RemoveWarningModal = (props: {
  ...
  return (
    <Modal title="Remove User Group?" onRequestClose={props.onRequestClose}>
      <Form
        {...warningForm}
        submitText={'Remove User Group'}
        cancelText={'Go Back'}
        onCancel={props.onRequestClose}
      >
        <em>
          Removing this user group will remove all read/write access to all the users in that group
          from this operation. Do you wish to continue?
        </em>
      </Form>
    </Modal>
  )
  ...
  return (
    <>
      {wiredPermissions.render((data) => {
        const matchingUsers = data.filter(({ userGroup }) =>
          normalizeName(userGroup).includes(normalizedSearchTerm),
        )
...
export default (props: { operationSlug: string; isAdmin: boolean }) => {
  ...
  return (
    <SettingsSection title="Operation User Groups" width="wide">
      {props.isAdmin && <NewUserGroupForm {...bus} operationSlug={props.operationSlug} />}
      <PermissionTable
