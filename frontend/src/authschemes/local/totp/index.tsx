...
const TotpEnabled = (props: { onSuccess: () => void }) => {
  ...
  return (
    <Form submitDanger submitText="Remove Multi-Factor Authentication" {...removeTotpForm}>
      Your account currently has Time-based One Time Password (TOTP) multi-factor authentication
      enabled
    </Form>
  )
}
...
const TotpDisabled = (props: { onSuccess: () => void }) => {
  ...
  return (
    <>
      ...
      <p>
        Multi-factor authentication adds an additional layer of security to your account by
        requiring more than just a password to log in.
      </p>
      <Button primary onClick={() => setShowTotpSetupModal(true)}>
...
const TotpSetupModal = (props: { onSuccess: () => void; onRequestClose: () => void }) => {
    ...
    <Modal title="Enable Multi-Factor Authentication" onRequestClose={props.onRequestClose}>
      {wiredGeneratedTotp.render((generatedTotp) => (
        <div className={cx('setup-modal')}>
          ...
          <p>
            Set up multi-factor auth with your authenticator app of choice by scanning the QR code
            below.
          </p>
          ...
          <p>
            To finalize setup, verify your app is generating correct multi-factor codes by entering
            a generated code below:
          </p>
...
const TotpSetupForm = (props: { onSuccess: () => void; onCancel: () => void; secret: string }) => {
