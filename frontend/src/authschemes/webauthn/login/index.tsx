...
import {
  convertToCredentialCreationOptions,
      ...
      await finishLogin(
        {
          ...
          response: {
            ...
            userHandle:
              pubKeyResponse.userHandle == null ? '' : encodeAsB64(pubKeyResponse.userHandle),
          },
        },
        isDiscoverable,
      )
    ...
    <div>
      {window.PublicKeyCredential && (
        <div className={cx('login-container')}>
          <div className={cx('mode-buttons')}>
            <ButtonGroup className={cx('row-buttons')}>
              <Button
                ...
                className={cx('mode-button-right')}
              ...
              <Button
                active={!isDiscoverable}
                className={cx('mode-button-left')}
                onClick={makeNonDiscoverable}
              >
                Username
              </Button>
            </ButtonGroup>
          ...
          {isDiscoverable ? (
            <div>
              <Form submitText="Login" {...registerProps} {...loginForm} autoFocus={true}></Form>
...
const RegisterModal = (props: { onRequestClose: () => void; isDiscoverable: boolean }) => {
  ...
  const registerForm = useForm({
    ...
    fields: [firstNameField, lastNameField, emailField, usernameField, keyNameField],
    handleSubmit: async () => {
