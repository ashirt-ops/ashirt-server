...
const base64UrlToBase64 = (input: string) => {
  ...
  let standardCharInput = input.replace(/-/g, '+').replace(/_/g, '/')
...
export const toByteArrayFromB64URL = (s: string) =>
  Uint8Array.from(atob(base64UrlToBase64(s)), toKeycode)
...
export const convertToCredentialCreationOptions = (
  ...
  const output: CredentialCreationOptions = {
    ...
    publicKey: {
      ...
      excludeCredentials: input.publicKey.excludeCredentials?.map((cred) => ({
        ...cred,
        id: toByteArrayFromB64URL(cred.id),
      })),
    },
  }
...
export const convertToPublicKeyCredentialRequestOptions = (
  input: ProvidedCredentialRequestOptions,
): PublicKeyCredentialRequestOptions => {
  const output: PublicKeyCredentialRequestOptions = {
    ...input.publicKey,
    challenge: toByteArrayFromB64URL(input.publicKey.challenge),
    allowCredentials: input.publicKey.allowCredentials?.map((listItem) => ({
      ...listItem,
      id: toByteArrayFromB64URL(listItem.id),
    })),
  }
