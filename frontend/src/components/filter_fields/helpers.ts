...
const itemizeBulletProps = (
  data: Array<BulletProps> | undefined,
  label: string,
  tf?: (v: BulletProps) => string,
): string => {
  const orNot = (v: BulletProps): string => {
    const trueVal = tf?.(v) ?? v.id
    return v.modifier == 'not' ? `!${trueVal}` : `${trueVal}`
  }
  ...
  Object.entries(tokens).forEach(([key, filterValues]) => {
    if (key == '') {
      ...
      opts.type = filterValues
        .map((fVal) =>
          findAndModify(supportedEvidenceCount, (t) => t.id == fVal.value, fVal.modifier),
        )
        .filter(isNotUndefined)
    } else if (key == 'linked') {
