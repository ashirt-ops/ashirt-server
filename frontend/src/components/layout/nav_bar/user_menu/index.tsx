...
const UserMenuDropdown = (props: { name: string; avatar: string }) => {
  ...
  return (
    <Menu>
      ...
      <MenuSeparator />
      <MenuItem icon={require('./settings.svg')} onClick={() => navigate('/account/profile')}>
        Account Settings
      </MenuItem>
      {isSuperAdmin && (
        <MenuItem icon={require('./admin.svg')} onClick={() => navigate('/admin')}>
          Admin
        </MenuItem>
      )}
      <MenuItem icon={require('./power.svg')} onClick={logoutAndRedirect}>
        Sign Out
      </MenuItem>
    </Menu>
  )
