...
export const UpsertOperationTagModal = (props: {
  ...
  return (
    <UpsertTagModal
      {...props}
      createFn={async (t) => {
        createTag({ ...t, operationSlug: props.operationSlug })
      }}
  ...
  return (
    ...
    >
      {wiredEvidence.render((evidence) => (
        <p>
          {evidence.length > 0 &&
            'This tag belongs to the following evidence and will be removed from them on deletion:'}
          {evidence.map((evi) => (
            <Link
