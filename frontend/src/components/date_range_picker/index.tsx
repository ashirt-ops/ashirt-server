  ...
  <div className={cx('popup')}>
    <div className={cx('shortcuts')}>
      ...
      {Object.keys(presetRanges).map((rangeName) => (
        <button key={rangeName} onClick={() => props.onSelectRange(presetRanges[rangeName])}>
          {rangeName}
        </button>
      ))}
    </div>
    <div className={cx('day-picker-area')}>
      ...
      />
      <Button primary className={cx('close-button')} onClick={props.onButtonClick}>
        Close
      </Button>
    </div>
  </div>
  ...
  return (
    ...
    >
      <Button
        ...
        title={stringifyRange(props.range)}
      />
    </Popover>
  )
}
