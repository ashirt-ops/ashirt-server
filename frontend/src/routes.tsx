...
const makeErrorDisplay =
  (title: string, message: string, withLoginLink = false) =>
  () => (
    <ErrorDisplay title={title} err={new Error(message)}>
      {withLoginLink && (
        <>
          <br />
          <NavLinkButton primary className={cx('return-button')} to={'/login'}>
            Return to login
          </NavLinkButton>
        </>
      )}
...
const makeAuthErr = (body: string, addLoginLink?: boolean) =>
  makeErrorDisplay('Authentication Error', body, addLoginLink ?? true)

const AuthRecoveryFailed = makeAuthErr(
  'Account recovery failed. The recovery code may be expired or incorrect. Please contact an administrator to provide a new url.',
)
...
const AuthIncomplete = makeAuthErr(
  'The system could not complete the login process. Please retry, and if the issue persists, please contact a system administrator.',
)
const AuthDisabled = makeAuthErr(
  'This account has been disabled. Please contact an adminstrator if you think this is an error',
  false,
)
const AuthNoRegistration = makeAuthErr(
  'Registration has been disabled. Please contract an administrator to request access.',
)
const NoAccess = makeErrorDisplay(
  'Access Error',
  'This url only works for users that are not logged in.',
  true,
)
