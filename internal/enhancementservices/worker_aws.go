...
func buildLambdaClient() error {
	...
	if err != nil {
		return errorwrap.WrapError("unable to establish an aws lambda session", err)
	}
...
func (w *awsConfigV1Worker) Build(workerName string, workerConfig []byte) error {
	...
	if err := json.Unmarshal([]byte(workerConfig), &awsConfig); err != nil {
		return errorwrap.WrapError("aws worker config is unparsable", err)
	}
...
func (w *awsConfigV1Worker) ProcessMetadata(evidenceID int64, payload *NewEvidencePayload) (*models.EvidenceMetadata, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("unable to construct body", err)
	}
	...
	out, err := lambdaClient.Invoke(context.Background(), &input)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to invoke lambda function", err)
	}
	if out.FunctionError != nil {
		return nil, errorwrap.WrapError("Lambda invocation failed", err)
	}
...
func (w *awsConfigV1Worker) ProcessEvent(payload interface{}) error {
	body, err := json.Marshal(payload)
	if err != nil {
		return errorwrap.WrapError("unable to construct body", err)
	}
	...
	if out, err := lambdaClient.Invoke(context.Background(), &input); err != nil {
		return errorwrap.WrapError("Unable to invoke lambda function", err)
	} else if out.FunctionError != nil {
		return errorwrap.WrapError("Lambda invocation failed", err)
	}
