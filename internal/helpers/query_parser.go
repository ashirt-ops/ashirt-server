	...
	for k, v := range tokenizeTimelineQuery(query) {
		...
		case "linked":
			if len(v) != 1 {
				...
				return timelineFilters, errors.BadInputErr(stderrors.New(errReason), errReason)
			}
...
func parseDateRangeString(dateRange string) (*filter.DateRange, error) {
	...
	if len(split) != 2 {
		...
		return nil, errors.BadInputErr(stderrors.New(errReason), errReason)
	}
...
func parseTime(str string, useEndOfDayIfTimeIsMissing bool) (time.Time, error) {
	...
	return time.Now(), errors.BadInputErr(
