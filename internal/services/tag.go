...
func CreateTag(ctx context.Context, db *database.Connection, i CreateTagInput) (*dtos.Tag, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to create tag", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyTagsOfOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unable to create tag", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot add new tag", errorwrap.DatabaseErr(err))
	}
	return &dtos.Tag{
		ID:          tagID,
		Name:        i.Name,
...
func CreateDefaultTag(ctx context.Context, db *database.Connection, i CreateDefaultTagInput) (*dtos.DefaultTag, error) {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return nil, errorwrap.WrapError("Unable to create default tag", errorwrap.UnauthorizedWriteErr(err))
	}
...
func MergeDefaultTags(ctx context.Context, db *database.Connection, i []CreateDefaultTagInput) error {
	if err := policyRequireWithAdminBypass(ctx, policy.AdminUsersOnly{}); err != nil {
		return errorwrap.WrapError("Unwilling to update default tag", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot update default tag", errorwrap.DatabaseErr(err))
	}
	return nil
}
...
func DeleteTag(ctx context.Context, db *database.Connection, i DeleteTagInput) error {
	operation, err := lookupOperation(db, i.OperationSlug)
	if err != nil {
		return errorwrap.WrapError("Unable to delete tag", errorwrap.UnauthorizedWriteErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyTagsOfOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to delete tag", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot delete tag", errorwrap.DatabaseErr(err))
	}
...
func DeleteDefaultTag(ctx context.Context, db *database.Connection, i DeleteDefaultTagInput) error {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return errorwrap.WrapError("Unwilling to delete default tag", errorwrap.UnauthorizedWriteErr(err))
	}

	err := db.Delete(sq.Delete("default_tags").Where(sq.Eq{"id": i.ID}))
	if err != nil {
		return errorwrap.WrapError("Cannot delete default tag", errorwrap.DatabaseErr(err))
	}
...
func ListTagDifference(ctx context.Context, db *database.Connection, i ListTagsDifferenceInput) (*dtos.TagDifference, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to list tag differences", err)
	}
	...
	if err := policyRequireWithAdminBypass(ctx,
		...
		return nil, errorwrap.WrapError("Unwilling to list tag differences", errorwrap.UnauthorizedReadErr(err))
	}

	sourceTags, err := listTagsForOperation(db, sourceOperation.ID)
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list tag differences", err)
	}
...
func ListTagDifferenceForEvidence(ctx context.Context, db *database.Connection, input ListTagDifferenceForEvidenceInput) (*dtos.TagDifference, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to list tag difference", err)
	}

	_, evidence, err := lookupOperationEvidence(db, input.SourceOperationSlug, input.SourceEvidenceUUID)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to get evidence for tagdiff", err)
	}

	tagMap, _, err := tagsForEvidenceByID(db, []int64{evidence.ID})
	if err != nil {
		return nil, errorwrap.WrapError("Cannot get evidence tags", err)
	}
...
func ListTagsForOperation(ctx context.Context, db *database.Connection, i ListTagsForOperationInput) ([]*dtos.TagWithUsage, error) {
	operation, err := lookupOperation(db, i.OperationSlug)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to list tags for operation", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policyRequireWithAdminBypass(ctx, policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list tags for operation", errorwrap.UnauthorizedReadErr(err))
	}
...
func listTagsForOperation(db *database.Connection, operationID int64) ([]*dtos.TagWithUsage, error) {
	...
	err := db.Select(&tags, sq.Select("tags.*").Column("count(tag_id) AS tag_count").
		...
		OrderBy("tags.id ASC"))
	if err != nil {
		return nil, errorwrap.WrapError("Cannot get tags for operation", errorwrap.DatabaseErr(err))
	}
...
func ListDefaultTags(ctx context.Context, db *database.Connection) ([]*dtos.DefaultTag, error) {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list default tags", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot get default tags", errorwrap.DatabaseErr(err))
	}
...
func UpdateTag(ctx context.Context, db *database.Connection, i UpdateTagInput) error {
	operation, err := lookupOperation(db, i.OperationSlug)
	if err != nil {
		return errorwrap.WrapError("Unable to update tag", errorwrap.UnauthorizedWriteErr(err))
	}

	if err := policyRequireWithAdminBypass(ctx, policy.CanModifyTagsOfOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to update tag", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot update tag", errorwrap.DatabaseErr(err))
	}
	return nil
}
