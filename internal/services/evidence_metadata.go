...
func CreateEvidenceMetadata(ctx context.Context, db *database.Connection, i EditEvidenceMetadataInput) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to create evidence metadata", errorwrap.UnauthorizedWriteErr(err))
	}
	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyEvidenceOfOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to create evidence metadata", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "Error 1062") {
			return errorwrap.WrapError("Couold not edit evidence metadata",
				errorwrap.SuggestiveDatabaseErr("This metadata source already exists", err),
			)
		}

		return errorwrap.WrapError("Could not create evidence metadata", errorwrap.DatabaseErr(err))
	}
...
func UpdateEvidenceMetadata(ctx context.Context, db *database.Connection, i EditEvidenceMetadataInput) error {
	operation, evidence, err := lookupOperationEvidence(db, i.OperationSlug, i.EvidenceUUID)
	if err != nil {
		return errorwrap.WrapError("Unable to edit evidence metadata", errorwrap.UnauthorizedWriteErr(err))
	}
	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyEvidenceOfOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to edit evidence metadata", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Could not edit evidence metadata", errorwrap.DatabaseErr(err))
	}
...
func ReadEvidenceMetadata(ctx context.Context, db *database.Connection, i ReadEvidenceMetadataInput) ([]*dtos.EvidenceMetadata, error) {
	operation, evidence, err := lookupOperationEvidence(db, i.OperationSlug, i.EvidenceUUID)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to read evidence metadata", errorwrap.UnauthorizedReadErr(err))
	}
	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to read evidence metadata", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot read evidence metadata", errorwrap.DatabaseErr(err))
	}
