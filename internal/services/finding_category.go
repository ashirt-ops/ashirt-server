...
func CreateFindingCategory(ctx context.Context, db *database.Connection, newCategory string) (*dtos.FindingCategory, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errorwrap.WrapError("Unable to create a new finding category", errorwrap.UnauthorizedWriteErr(err))
	}
...
func DeleteFindingCategory(ctx context.Context, db *database.Connection, i DeleteFindingCategoryInput) error {
	if err := isAdmin(ctx); err != nil {
		return errorwrap.WrapError("Unable to delete a finding category", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err := db.Update(query); err != nil {
		return errorwrap.WrapError("Cannot delete finding category", errorwrap.DatabaseErr(err))
	}
...
func ListFindingCategories(ctx context.Context, db *database.Connection, includeDeleted bool) (interface{}, error) {
	...
	if err := db.Select(&categories, query); err != nil {
		return nil, errorwrap.WrapError("Cannot list finding categories", errorwrap.DatabaseErr(err))
	}
...
func UpdateFindingCategory(ctx context.Context, db *database.Connection, i UpdateFindingCategoryInput) error {
	if err := isAdmin(ctx); err != nil {
		return errorwrap.WrapError("Unable to update the finding category", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot update finding category", errorwrap.DatabaseErr(err))
	}
	return nil
}
