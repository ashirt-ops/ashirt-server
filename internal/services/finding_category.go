...
func CreateFindingCategory(ctx context.Context, db *database.Connection, newCategory string) (*dtos.FindingCategory, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errors.WrapError("Unable to create a new finding category", errors.UnauthorizedWriteErr(err))
	}
...
func DeleteFindingCategory(ctx context.Context, db *database.Connection, i DeleteFindingCategoryInput) error {
	if err := isAdmin(ctx); err != nil {
		return errors.WrapError("Unable to delete a finding category", errors.UnauthorizedWriteErr(err))
	}
	...
	if err := db.Update(query); err != nil {
		return errors.WrapError("Cannot delete finding category", errors.DatabaseErr(err))
	}
...
func ListFindingCategories(ctx context.Context, db *database.Connection, includeDeleted bool) (interface{}, error) {
	...
	if err := db.Select(&categories, query); err != nil {
		return nil, errors.WrapError("Cannot list finding categories", errors.DatabaseErr(err))
	}
...
func UpdateFindingCategory(ctx context.Context, db *database.Connection, i UpdateFindingCategoryInput) error {
	if err := isAdmin(ctx); err != nil {
		return errors.WrapError("Unable to update the finding category", errors.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errors.WrapError("Cannot update finding category", errors.DatabaseErr(err))
	}
	return nil
}
