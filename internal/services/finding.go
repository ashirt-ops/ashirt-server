...
func CreateFinding(ctx context.Context, db *database.Connection, i CreateFindingInput) (*dtos.Finding, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to create finding", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if i.Title == "" {
		return nil, errorwrap.MissingValueErr("Title")
	}

	if i.Category == "" {
		return nil, errorwrap.MissingValueErr("Category")
	}

	useCategoryID, err := getFindingCategoryID(i.Category, db.Select)

	if err != nil {
		return nil, errorwrap.WrapError("Unable create finding", err)
	}
	if useCategoryID == nil {
		return nil, errorwrap.BadInputErr(errors.New("no such category"), "Unknown Category")
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to insert finding", errorwrap.DatabaseErr(err))
	}
...
func DeleteFinding(ctx context.Context, db *database.Connection, i DeleteFindingInput) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to delete finding", errorwrap.UnauthorizedWriteErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyFindingsOfOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to delete finding", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot delete finding", errorwrap.DatabaseErr(err))
	}
...
func ListFindingsForOperation(ctx context.Context, db *database.Connection, i ListFindingsForOperationInput) ([]*dtos.Finding, error) {
	operation, err := lookupOperation(db, i.OperationSlug)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to list findings for operation", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list findings for operation", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list findings for operation", errorwrap.DatabaseErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot find all tags", errorwrap.DatabaseErr(err))
	}
...
func ReadFinding(ctx context.Context, db *database.Connection, i ReadFindingInput) (*dtos.Finding, error) {
	operation, finding, err := lookupOperationFinding(db, i.OperationSlug, i.FindingUUID)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to read finding", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to read finding", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot load evidence for finding", errorwrap.DatabaseErr(err))
	}

	_, allTags, err := tagsForEvidenceByID(db, evidenceIDs)
	if err != nil {
		return nil, errorwrap.WrapError("Cannot load tags for evidence", errorwrap.DatabaseErr(err))
	}
	...
	if finding.CategoryID != nil {
		...
		if err != nil {
			return nil, errorwrap.WrapError("Cannot load finding category for finding", errorwrap.DatabaseErr(err))
		}
	}
...
func UpdateFinding(ctx context.Context, db *database.Connection, i UpdateFindingInput) error {
	operation, finding, err := lookupOperationFinding(db, i.OperationSlug, i.FindingUUID)
	if err != nil {
		return errorwrap.WrapError("Unable to lookup operation", errorwrap.UnauthorizedWriteErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyFindingsOfOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Failed permission check", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Unable to update database", errorwrap.UnauthorizedWriteErr(err))
	}
	return nil
}
...
func AddEvidenceToFinding(ctx context.Context, db *database.Connection, i AddEvidenceToFindingInput) error {
	operation, finding, err := lookupOperationFinding(db, i.OperationSlug, i.FindingUUID)
	if err != nil {
		return errorwrap.WrapError("Unable to add evidence to finding", errorwrap.UnauthorizedWriteErr(err))
	}
...
func batchAddEvidenceToFinding(db *database.Connection, evidenceUUIDs []string, operationID int64, findingID int64) error {
	...
	if err := db.Select(&evidence, buildQueryForEvidenceFromUUIDs(evidenceUUIDs)); err != nil {
		return errorwrap.WrapError("Unable to get evidence from uuids", err)
	}
	evidenceIDs := []int64{}
