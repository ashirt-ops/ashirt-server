...
func (i ModifyUserGroupInput) validateUserGroupInput() error {
	if i.Slug == "" {
		return errorwrap.MissingValueErr("Slug")
	}
	return nil
}
...
func AddUsersToGroup(tx *database.Transactable, userSlugs []string, groupID int64) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to add users to group", errorwrap.DatabaseErr(err))
	}
	return nil
}
...
func CreateUserGroup(ctx context.Context, db *database.Connection, i CreateUserGroupInput) (*dtos.UserGroup, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errorwrap.WrapError("Unwilling to create a user group", errorwrap.UnauthorizedReadErr(err))
	}

	cleanSlug := SanitizeSlug(i.Slug)
	if cleanSlug == "" {
		return nil, errorwrap.BadInputErr(errors.New("Unable to create operation. Invalid operation slug"), "Slug must contain english letters or numbers")
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Error creating user group", errorwrap.BadInputErr(err, "A user group with this slug already exists; please choose another name"))
	}
...
func ModifyUserGroup(ctx context.Context, db *database.Connection, i ModifyUserGroupInput) (*dtos.UserGroup, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errorwrap.WrapError("Unwilling to modify a user group", errorwrap.UnauthorizedReadErr(err))
	}

	if err := i.validateUserGroupInput(); err != nil {
		return nil, errorwrap.WrapError("Unable to modify user group", errorwrap.BadInputErr(err, "Unable to modify user group due to bad input"))
	}

	userGroup, err := lookupUserGroup(db, i.Slug)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to modify user group", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Error creating user group", errorwrap.BadInputErr(err, "A user group with this name already exists; please choose another name"))
	}
...
func DeleteUserGroup(ctx context.Context, db *database.Connection, slug string) error {
	if err := isAdmin(ctx); err != nil {
		return errorwrap.WrapError("Unwilling to delete a user group", errorwrap.UnauthorizedReadErr(err))
	}
	userGroup, err := lookupUserGroup(db, slug)
	if err != nil {
		return errorwrap.WrapError("Unable to delete user group", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot delete user group", errorwrap.DatabaseErr(err))
	}
...
func ListUserGroupsForAdmin(ctx context.Context, db *database.Connection, i ListUserGroupsForAdminInput) ([]dtos.UserGroupAdminView, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list user groups", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to list user groups", errorwrap.DatabaseErr(err))
	}
...
func GetSlugMap(db *database.Connection, i ListUserGroupsForAdminInput) (SlugMap, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("unable to get map of user IDs to group IDs from database", errorwrap.DatabaseErr(err))
	}
...
func ListUserGroupsForOperation(ctx context.Context, db *database.Connection, i ListUserGroupsForOperationInput) ([]*dtos.UserGroupOperationRole, error) {
	...
	if err := policyRequireWithAdminBypass(ctx, policy.CanListUserGroupsOfOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list usergroups", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list user groups for operation", errorwrap.DatabaseErr(err))
	}
...
func ListUserGroups(ctx context.Context, db *database.Connection, i ListUserGroupsInput) ([]*dtos.UserGroupAdminView, error) {
	...
	err = db.Select(&userGroups, query)
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list user groups", errorwrap.DatabaseErr(err))
	}
