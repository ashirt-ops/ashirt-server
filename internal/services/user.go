...
func (cui CreateUserInput) validate() error {
	...
	if cui.FirstName == "" {
		return errors.MissingValueErr("First Name")
	}
	if cui.LastName == "" {
		return errors.MissingValueErr("Last Name")
	}
	if cui.Email == "" {
		return errors.MissingValueErr("Email")
	}
	return nil
}
...
func CreateHeadlessUser(ctx context.Context, db *database.Connection, i CreateUserInput) (*dtos.CreateUserOutput, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errors.WrapError("Unable to create new headless user", errors.UnauthorizedWriteErr(err))
	}
	i.Headless = true
...
func CreateUser(db *database.Connection, i CreateUserInput) (*dtos.CreateUserOutput, error) {
	...
	if validationErr != nil {
		return nil, errors.WrapError("Unable to create new user", validationErr)
	}
		...
		if err != nil {
			if database.IsAlreadyExistsError(err) {
				if strings.Contains(err.Error(), "users.unique_email") { // not sure how else to check if this is a duplicate slug vs a duplicate email address
					return nil, errors.WrapError("Unable to insert new user", errors.DatabaseErr(err))
				}

				if attemptNumber > 5 {
					return nil, errors.WrapError("Unable to create new user after many attempts", errors.DatabaseErr(err))
				}
...
func DeleteUser(ctx context.Context, db *database.Connection, slug string) error {
	if !middleware.IsAdmin(ctx) {
		return errors.WrapError("Unwilling to delete user", errors.UnauthorizedWriteErr(fmt.Errorf("Requesting user is not an admin")))
	}

	userID, err := userSlugToUserID(db, slug)
	if err != nil {
		return errors.WrapError("Unable to delete user", errors.DatabaseErr(err))
	}

	if userID == middleware.UserID(ctx) {
		return errors.BadInputErr(fmt.Errorf("User is trying to delete themself"), "Users cannot delete themselves")
	}
	...
	if disableErr != nil {
		return errors.WrapError("Could not set user to disabled prior to deletion", disableErr)
	}
	...
	if err != nil {
		return errors.WrapError("Cannot delete user", errors.DatabaseErr(err))
	}
...
func ListEvidenceCreatorsForOperation(ctx context.Context, db *database.Connection, i ListEvidenceCreatorsForOperationInput) ([]*dtos.User, error) {
	...
	if err != nil {
		return nil, errors.WrapError("Unable to list evidence for an operation", errors.UnauthorizedReadErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errors.WrapError("Unwilling to list evidence for an operation", errors.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errors.WrapError("Cannot list evidence creators for an operation", errors.DatabaseErr(err))
	}
...
func ListUsersForAdmin(ctx context.Context, db *database.Connection, i ListUsersForAdminInput) (*dtos.PaginationWrapper, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errors.WrapError("Unwilling to list users", errors.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errors.WrapError("Cannot list users for admin", errors.DatabaseErr(err))
	}
...
func ListUsersForOperation(ctx context.Context, db *database.Connection, i ListUsersForOperationInput) ([]*dtos.UserOperationRole, error) {
	...
	if err != nil {
		return nil, errors.WrapError("Cannot list users for operation", errors.DatabaseErr(err))
	}
...
func ListUsers(ctx context.Context, db *database.Connection, i ListUsersInput) ([]*dtos.User, error) {
	...
	err := db.Select(&users, query)
	if err != nil {
		return nil, errors.WrapError("Cannot list users", errors.DatabaseErr(err))
	}
...
func prepListUsersForOperation(ctx context.Context, db *database.Connection, i ListUsersForOperationInput) (*sq.SelectBuilder, error) {
	operation, err := lookupOperation(db, i.OperationSlug)
	if err != nil {
		return nil, errors.WrapError("Unable to list users for operation", errors.UnauthorizedReadErr(err))
	}

	if err := policyRequireWithAdminBypass(ctx, policy.CanListUsersOfOperation{OperationID: operation.ID}); err != nil {
		return nil, errors.WrapError("Unwilling to list users for operation", errors.UnauthorizedReadErr(err))
	}
...
func ReadUser(ctx context.Context, db *database.Connection, userSlug string, supportedAuthSchemes *[]dtos.SupportedAuthScheme) (*dtos.UserOwnView, error) {
	...
	if err != nil {
		return nil, errors.WrapError("Unable to read user", errors.DatabaseErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadDetailedUser{UserID: userID}); err != nil {
		return nil, errors.WrapError("Unwilling to read user", errors.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errors.WrapError("Cannot read user", errors.DatabaseErr(err))
	}
...
func UpdateUserProfile(ctx context.Context, db *database.Connection, i UpdateUserProfileInput) error {
	...
	if userID, err = SelfOrSlugToUserID(ctx, db, i.UserSlug); err != nil {
		return errors.WrapError("Unable to update user profile", errors.DatabaseErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyUser{UserID: userID}); err != nil {
		return errors.WrapError("Unwilling to update user profile", errors.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errors.WrapError("Cannot update user profile", errors.DatabaseErr(err))
	}
	return nil
}
...
func DeleteSessionsForUserSlug(ctx context.Context, db *database.Connection, userSlug string) error {
	if !middleware.IsAdmin(ctx) {
		return errors.UnauthorizedReadErr(fmt.Errorf("Requesting user is not an admin"))
	}

	userID, err := userSlugToUserID(db, userSlug)
	if err != nil {
		return errors.WrapError("Unable to delete user session", errors.DatabaseErr(err))
	}
...
func SetUserFlags(ctx context.Context, db *database.Connection, i SetUserFlagsInput) error {
	if !middleware.IsAdmin(ctx) {
		return errors.WrapError("Unwilling to set user flag", errors.UnauthorizedReadErr(fmt.Errorf("Requesting user is not an admin")))
	}

	targetUser, err := db.RetrieveUserWithAuthDataBySlug(i.Slug)
	if err != nil {
		return errors.WrapError("Cannot set user flags", errors.DatabaseErr(err))
	}
...
func deleteSessionsForUserID(db *database.Connection, userID int64) error {
	...
	if err := db.Exec("DELETE FROM sessions WHERE user_id = ?", userID); err != nil {
		return errors.WrapError("Cannot delete user session", errors.DatabaseErr(err))
	}
	return nil
}
...
func validateAdminCanModifyFlag(ctx context.Context, targetUser models.UserWithAuthData, flagsToUpdate SetUserFlagsInput) error {
	...
	if flagsToUpdate.Admin != nil {
		...
		if targetUserIsSelf && valueUpdated {
			return stderrors.New("Admins cannot alter their own admin status")
		}
		if targetUserIsHeadless && *flagsToUpdate.Admin {
			return stderrors.New("Headless users cannot be granted admin status")
		}
	}
	...
	if flagsToUpdate.Disabled != nil {
		...
		if targetUserIsSelf && valueUpdated {
			return stderrors.New("Admins cannot disable themselves")
		}
	}
