...
import (
	...
	localauth "github.com/ashirt-ops/ashirt-server/internal/authschemes/localauth/constants"
...
func (cui CreateUserInput) validate() error {
	if cui.Slug == "" {
		return errorwrap.MissingValueErr("User Slug")
	}
	if cui.FirstName == "" {
		return errorwrap.MissingValueErr("First Name")
	}
	if cui.LastName == "" {
		return errorwrap.MissingValueErr("Last Name")
	}
	if cui.Email == "" {
		return errorwrap.MissingValueErr("Email")
	}
	return nil
}
...
func CreateHeadlessUser(ctx context.Context, db *database.Connection, i CreateUserInput) (*dtos.CreateUserOutput, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errorwrap.WrapError("Unable to create new headless user", errorwrap.UnauthorizedWriteErr(err))
	}
	i.Headless = true
...
func CreateUser(db *database.Connection, i CreateUserInput) (*dtos.CreateUserOutput, error) {
	...
	if validationErr != nil {
		return nil, errorwrap.WrapError("Unable to create new user", validationErr)
	}
		...
		if err != nil {
			if database.IsAlreadyExistsError(err) {
				if strings.Contains(err.Error(), "users.unique_email") { // not sure how else to check if this is a duplicate slug vs a duplicate email address
					return nil, errorwrap.WrapError("Unable to insert new user", errorwrap.DatabaseErr(err))
				}

				if attemptNumber > 5 {
					return nil, errorwrap.WrapError("Unable to create new user after many attempts", errorwrap.DatabaseErr(err))
				}
...
func DeleteUser(ctx context.Context, db *database.Connection, slug string) error {
	if !middleware.IsAdmin(ctx) {
		return errorwrap.WrapError("Unwilling to delete user", errorwrap.UnauthorizedWriteErr(fmt.Errorf("Requesting user is not an admin")))
	}

	userID, err := userSlugToUserID(db, slug)
	if err != nil {
		return errorwrap.WrapError("Unable to delete user", errorwrap.DatabaseErr(err))
	}

	if userID == middleware.UserID(ctx) {
		return errorwrap.BadInputErr(fmt.Errorf("User is trying to delete themself"), "Users cannot delete themselves")
	}
	...
	if disableErr != nil {
		return errorwrap.WrapError("Could not set user to disabled prior to deletion", disableErr)
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot delete user", errorwrap.DatabaseErr(err))
	}
...
func ListEvidenceCreatorsForOperation(ctx context.Context, db *database.Connection, i ListEvidenceCreatorsForOperationInput) ([]*dtos.User, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to list evidence for an operation", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list evidence for an operation", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list evidence creators for an operation", errorwrap.DatabaseErr(err))
	}
...
func ListUsersForAdmin(ctx context.Context, db *database.Connection, i ListUsersForAdminInput) (*dtos.PaginationWrapper, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list users", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list users for admin", errorwrap.DatabaseErr(err))
	}

	usersDTO := []*dtos.UserAdminView{}
	for _, user := range users {
		// Group_Concat will return null if there are no authentication schemes listed. The below forces the schemes into a slice to avoid errorwrap.
		authSchemes := []string{}
...
func ListUsersForOperation(ctx context.Context, db *database.Connection, i ListUsersForOperationInput) ([]*dtos.UserOperationRole, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list users for operation", errorwrap.DatabaseErr(err))
	}
...
func ListUsers(ctx context.Context, db *database.Connection, i ListUsersInput) ([]*dtos.User, error) {
	...
	err := db.Select(&users, query)
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list users", errorwrap.DatabaseErr(err))
	}
...
func prepListUsersForOperation(ctx context.Context, db *database.Connection, i ListUsersForOperationInput) (*sq.SelectBuilder, error) {
	operation, err := lookupOperation(db, i.OperationSlug)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to list users for operation", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policyRequireWithAdminBypass(ctx, policy.CanListUsersOfOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list users for operation", errorwrap.UnauthorizedReadErr(err))
	}
...
func ReadUser(ctx context.Context, db *database.Connection, userSlug string, supportedAuthSchemes *[]dtos.SupportedAuthScheme) (*dtos.UserOwnView, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to read user", errorwrap.DatabaseErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadDetailedUser{UserID: userID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to read user", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot read user", errorwrap.DatabaseErr(err))
	}
...
func UpdateUserProfile(ctx context.Context, db *database.Connection, i UpdateUserProfileInput) error {
	...
	if userID, err = SelfOrSlugToUserID(ctx, db, i.UserSlug); err != nil {
		return errorwrap.WrapError("Unable to update user profile", errorwrap.DatabaseErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyUser{UserID: userID}); err != nil {
		return errorwrap.WrapError("Unwilling to update user profile", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot update user profile", errorwrap.DatabaseErr(err))
	}
	return nil
}
...
func DeleteSessionsForUserSlug(ctx context.Context, db *database.Connection, userSlug string) error {
	if !middleware.IsAdmin(ctx) {
		return errorwrap.UnauthorizedReadErr(fmt.Errorf("Requesting user is not an admin"))
	}

	userID, err := userSlugToUserID(db, userSlug)
	if err != nil {
		return errorwrap.WrapError("Unable to delete user session", errorwrap.DatabaseErr(err))
	}
...
func SetUserFlags(ctx context.Context, db *database.Connection, i SetUserFlagsInput) error {
	if !middleware.IsAdmin(ctx) {
		return errorwrap.WrapError("Unwilling to set user flag", errorwrap.UnauthorizedReadErr(fmt.Errorf("Requesting user is not an admin")))
	}

	targetUser, err := db.RetrieveUserWithAuthDataBySlug(i.Slug)
	if err != nil {
		return errorwrap.WrapError("Cannot set user flags", errorwrap.DatabaseErr(err))
	}
	...
	if len(valuesToUpdate) > 0 {
		...
		if err != nil {
			return errorwrap.DatabaseErr(err)
		}
...
func deleteSessionsForUserID(db *database.Connection, userID int64) error {
	...
	if err := db.Exec("DELETE FROM sessions WHERE user_id = ?", userID); err != nil {
		return errorwrap.WrapError("Cannot delete user session", errorwrap.DatabaseErr(err))
	}
	return nil
}
