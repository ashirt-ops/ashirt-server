...
func CreateOperationVar(ctx context.Context, db *database.Connection, i CreateOperationVarInput) (*dtos.OperationVar, error) {
	...
	if err := policy.Require(middleware.Policy(ctx), policy.CanCreateOpVars{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unable to create operation variable", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if cleanSlug == "" {
		return nil, errorwrap.BadInputErr(errors.New("Unable to create operation variable. Invalid operation variable slug"), "Slug must contain english letters or numbers")
	}
	...
	for _, varInOperation := range listOfVarsInOperation {
		if varInOperation.Name == formattedName {
			return nil, errorwrap.BadInputErr(errors.New("Unable to create operation variable. Invalid operation variable name"), "A variable with this name already exists in the operation")
		}
	}
	...
	if err != nil {
		...
		return nil, errorwrap.BadInputErr(errorwrap.WrapError(errMessage, errorwrap.DatabaseErr(err)), errMessage)
	}
...
func DeleteOperationVar(ctx context.Context, db *database.Connection, varSlug string, operationSlug string) error {
	...
	if err := policyRequireWithAdminBypass(ctx, policy.CanDeleteOpVars{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to delete operation variable", errorwrap.UnauthorizedWriteErr(err))
	}

	err = db.Delete(sq.Delete("operation_vars").Where(sq.Eq{"slug": varSlug}))
	if err != nil {
		return errorwrap.WrapError("Cannot delete operation variable", errorwrap.DatabaseErr(err))
	}
...
func ListOperationVars(ctx context.Context, db *database.Connection, operationSlug string) ([]*dtos.OperationVar, error) {
	...
	if err := policy.Require(middleware.Policy(ctx), policy.CanViewOpVars{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list operation variables", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list operation variables", errorwrap.DatabaseErr(err))
	}
...
func UpdateOperationVar(ctx context.Context, db *database.Connection, i UpdateOperationVarInput) error {
	...
	for _, varInOperation := range listOfVarsInOperation {
		if varInOperation.Name == formattedName {
			return errorwrap.BadInputErr(errors.New("Unable to update operation variable. Invalid operation variable name"), "A variable with this name already exists in the operation")
		}
	}
	...
	if err != nil {
		...
		return errorwrap.BadInputErr(errorwrap.WrapError(errMessage, errorwrap.DatabaseErr(err)), errMessage)
	}
