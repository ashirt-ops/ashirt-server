...
func CreateOperation(ctx context.Context, db *database.Connection, i CreateOperationInput) (*dtos.Operation, error) {
	if err := policy.Require(middleware.Policy(ctx), policy.CanCreateOperations{}); err != nil {
		return nil, errorwrap.WrapError("Unable to create operation", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if i.Slug == "" {
		return nil, errorwrap.MissingValueErr("Slug")
	}
	...
	if err != nil {
		if database.IsAlreadyExistsError(err) {
			return nil, errorwrap.WrapError("Unable to create operation. Operation slug already exists.", errorwrap.BadInputErr(err, "An operation with this slug already exists"))
		}
		return nil, errorwrap.WrapError("Unable to add new operation", errorwrap.DatabaseErr(err))
	}
...
func DeleteOperation(ctx context.Context, db *database.Connection, contentStore contentstore.Store, slug string) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to delete operation", errorwrap.UnauthorizedWriteErr(err))
	}

	if err := policyRequireWithAdminBypass(ctx, policy.CanDeleteOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to delete operation", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	g.Go(func() error {
				...
				for _, evi := range evidence {
					...
					g.Go(func() error {
						...
						if err != nil {
							...
							return errorwrap.DeleteErr(err)
						}
						return nil
					})
				}
		...
		if err != nil {
			...
			return errorwrap.WrapError("Cannot delete operation", errorwrap.DatabaseErr(err))
		}
		return nil
	})
...
func ListOperations(ctx context.Context, db *database.Connection) ([]*dtos.Operation, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot get user operation preferences", errorwrap.DatabaseErr(err))
	}
...
func ReadOperation(ctx context.Context, db *database.Connection, operationSlug string) (*dtos.Operation, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to read operation", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policyRequireWithAdminBypass(ctx, policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to read operation", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot read operation", errorwrap.DatabaseErr(err))
	}
...
func UpdateOperation(ctx context.Context, db *database.Connection, i UpdateOperationInput) error {
	...
	if err != nil {
		return errorwrap.WrapError("Cannot update operation", errorwrap.DatabaseErr(err))
	}
	return nil
}
...
func ListOperationsForAdmin(ctx context.Context, db *database.Connection) ([]*dtos.Operation, error) {
	if err := isAdmin(ctx); err != nil {
		return nil, errorwrap.UnauthorizedReadErr(err)
	}
...
func listAllOperations(ctx context.Context, db *database.Connection) ([]OperationWithID, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list all operations", errorwrap.DatabaseErr(err))
	}
...
func SetFavoriteOperation(ctx context.Context, db *database.Connection, i SetFavoriteInput) error {
	operation, err := lookupOperation(db, i.OperationSlug)
	if err != nil {
		return errorwrap.WrapError("Unable to read operation", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to read operation", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot set operation as favorite", errorwrap.DatabaseErr(err))
	}
