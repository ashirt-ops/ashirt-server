...
func CreateEvidence(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i CreateEvidenceInput) (*dtos.Evidence, error) {
	...
	if err != nil {
		return nil, errors.WrapError("Unable to create evidence", errors.UnauthorizedWriteErr(err))
	}
	...
	if err := ensureTagIDsBelongToOperation(db, i.TagIDs, operation); err != nil {
		return nil, errors.BadInputErr(err, err.Error())
	}
	...
	if i.Content != nil {
		...
		if err != nil {
			...
			return nil, errors.WrapError("Unable to upload evidence", errors.UploadErr(err))
		}
	}
	...
	if err != nil {
		return nil, errors.WrapError("Could not create evidence and tags", errors.DatabaseErr(err))
	}
...
func DeleteEvidence(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i DeleteEvidenceInput) error {
	...
	if err != nil {
		return errors.WrapError("Unable to delete evidence", errors.UnauthorizedWriteErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyEvidenceOfOperation{OperationID: operation.ID}); err != nil {
		return errors.WrapError("Unwilling to delete evidence", errors.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errors.WrapError("Cannot delete evidence", errors.DatabaseErr(err))
	}

	if err = deleteEvidenceContent(contentStore, *evidence); err != nil {
		return errors.WrapError("Cannot delete evidence content", errors.DeleteErr(err))
	}
...
func ListEvidenceForFinding(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i ListEvidenceForFindingInput) ([]dtos.Evidence, error) {
	...
	if err != nil {
		return nil, errors.WrapError("Unable to list evidence for finding", errors.UnauthorizedReadErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errors.WrapError("Unwilling to list evidence for finding", errors.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errors.WrapError("Cannot list evidence for finding", errors.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errors.WrapError("Cannot get tags for evidnece", errors.UnauthorizedReadErr(err))
	}
...
func ListEvidenceForOperation(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i ListEvidenceForOperationInput) ([]*dtos.Evidence, error) {
	...
	if err != nil {
		return nil, errors.WrapError("Cannot list evidence for an operation", errors.DatabaseErr(err))
	}
	...
	tagsByEvidenceID, _, err := tagsForEvidenceByID(db, evidenceIDs)
	if err != nil {
		return nil, errors.WrapError("Cannot get tags for evidence", errors.DatabaseErr(err))
	}
...
func SendURLData(ctx context.Context, db *database.Connection, contentStore *contentstore.S3Store, i ReadEvidenceInput) (*contentstore.URLData, error) {
	operation, evidence, err := lookupOperationEvidence(db, i.OperationSlug, i.EvidenceUUID)
	if err != nil {
		return nil, errors.WrapError("Unable to read evidence", errors.UnauthorizedReadErr(err))
	}
	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errors.WrapError("Unwilling to read evidence", errors.UnauthorizedReadErr(err))
	}
	urlData, err := contentStore.SendURLData(evidence.FullImageKey)
	if err != nil {
		return nil, errors.WrapError("Unable to get image URL", errors.ServerErr(err))
	}
...
func ReadEvidence(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i ReadEvidenceInput) (*ReadEvidenceOutput, error) {
	...
	if i.LoadPreview {
		...
		if err != nil {
			return nil, errors.WrapError("Cannot read evidence preview", err)
		}
	}
	...
	if i.LoadMedia {
		...
		if err != nil {
			return nil, errors.WrapError("Cannot read evidence media", err)
		}
	}
...
func UpdateEvidence(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i UpdateEvidenceInput) error {
	operation, evidence, err := lookupOperationEvidence(db, i.OperationSlug, i.EvidenceUUID)
	if err != nil {
		return errors.WrapError("Unable to update evidence", errors.UnauthorizedWriteErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyEvidenceOfOperation{OperationID: operation.ID}); err != nil {
		return errors.WrapError("Unwilling to update evidence", errors.UnauthorizedWriteErr(err))
	}

	if err := ensureTagIDsBelongToOperation(db, i.TagsToAdd, operation); err != nil {
		return errors.WrapError("Unable to update evidence", errors.BadInputErr(err, err.Error()))
	}
	...
	if i.Content != nil {
		...
		case "terminal-recording":
			...
			if err != nil {
				return errors.WrapError("Cannot update evidence content", errors.BadInputErr(err, "Failed to process content"))
			}
			keys = &processedKeys
		...
		case "image":
			fallthrough
		default:
			err := stderrors.New("Content cannot be updated")
	...
	if err != nil {
		return errors.WrapError("Cannot update evidence", errors.DatabaseErr(err))
	}
...
func MoveEvidence(ctx context.Context, db *database.Connection, i MoveEvidenceInput) error {
	...
	if err != nil {
		return errors.WrapError("Unable to move evidence (src)", errors.UnauthorizedReadErr(err))
	}

	destinationOperation, err := lookupOperation(db, i.TargetOperationSlug)
	if err != nil {
		return errors.WrapError("Unable to move evidence (dst op)", errors.UnauthorizedReadErr(err))
	}
	...
	if err := policyRequireWithAdminBypass(ctx,
		...
		return errors.WrapError("Unwilling to move evidence", errors.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errors.WrapError("Unable to list tag differences for moving", err)
	}
	...
	if err != nil {
		return errors.WrapError("Cannot move evidence", err)
	}
