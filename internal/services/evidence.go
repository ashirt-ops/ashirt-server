...
import (
	...
	"github.com/ashirt-ops/ashirt-server/internal/enhancementservices"
...
func CreateEvidence(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i CreateEvidenceInput) (*dtos.Evidence, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to create evidence", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err := ensureTagIDsBelongToOperation(db, i.TagIDs, operation); err != nil {
		return nil, errorwrap.BadInputErr(err, err.Error())
	}
	...
	if i.Content != nil {
		...
		if err != nil {
			if httpErr, ok := err.(*errorwrap.HTTPError); ok {
				return nil, httpErr
			}
			return nil, errorwrap.WrapError("Unable to upload evidence", errorwrap.UploadErr(err))
		}
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Could not create evidence and tags", errorwrap.DatabaseErr(err))
	}
...
func DeleteEvidence(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i DeleteEvidenceInput) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to delete evidence", errorwrap.UnauthorizedWriteErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyEvidenceOfOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to delete evidence", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot delete evidence", errorwrap.DatabaseErr(err))
	}

	if err = deleteEvidenceContent(contentStore, *evidence); err != nil {
		return errorwrap.WrapError("Cannot delete evidence content", errorwrap.DeleteErr(err))
	}
...
func ListEvidenceForFinding(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i ListEvidenceForFindingInput) ([]dtos.Evidence, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to list evidence for finding", errorwrap.UnauthorizedReadErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to list evidence for finding", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list evidence for finding", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot get tags for evidnece", errorwrap.UnauthorizedReadErr(err))
	}
...
func ListEvidenceForOperation(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i ListEvidenceForOperationInput) ([]*dtos.Evidence, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Cannot list evidence for an operation", errorwrap.DatabaseErr(err))
	}
	...
	tagsByEvidenceID, _, err := tagsForEvidenceByID(db, evidenceIDs)
	if err != nil {
		return nil, errorwrap.WrapError("Cannot get tags for evidence", errorwrap.DatabaseErr(err))
	}
...
func SendURLData(ctx context.Context, db *database.Connection, contentStore *contentstore.S3Store, i ReadEvidenceInput) (*contentstore.URLData, error) {
	operation, evidence, err := lookupOperationEvidence(db, i.OperationSlug, i.EvidenceUUID)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to read evidence", errorwrap.UnauthorizedReadErr(err))
	}
	if err := policy.Require(middleware.Policy(ctx), policy.CanReadOperation{OperationID: operation.ID}); err != nil {
		return nil, errorwrap.WrapError("Unwilling to read evidence", errorwrap.UnauthorizedReadErr(err))
	}
	urlData, err := contentStore.SendURLData(evidence.FullImageKey)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to get image URL", errorwrap.ServerErr(err))
	}
...
func ReadEvidence(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i ReadEvidenceInput) (*ReadEvidenceOutput, error) {
	...
	if i.LoadPreview {
		...
		if err != nil {
			return nil, errorwrap.WrapError("Cannot read evidence preview", err)
		}
	}
	...
	if i.LoadMedia {
		...
		if err != nil {
			return nil, errorwrap.WrapError("Cannot read evidence media", err)
		}
	}
...
func UpdateEvidence(ctx context.Context, db *database.Connection, contentStore contentstore.Store, i UpdateEvidenceInput) error {
	operation, evidence, err := lookupOperationEvidence(db, i.OperationSlug, i.EvidenceUUID)
	if err != nil {
		return errorwrap.WrapError("Unable to update evidence", errorwrap.UnauthorizedWriteErr(err))
	}

	if err := policy.Require(middleware.Policy(ctx), policy.CanModifyEvidenceOfOperation{OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to update evidence", errorwrap.UnauthorizedWriteErr(err))
	}

	if err := ensureTagIDsBelongToOperation(db, i.TagsToAdd, operation); err != nil {
		return errorwrap.WrapError("Unable to update evidence", errorwrap.BadInputErr(err, err.Error()))
	}
	...
	if i.Content != nil {
		...
		case "terminal-recording":
			...
			if err != nil {
				return errorwrap.WrapError("Cannot update evidence content", errorwrap.BadInputErr(err, "Failed to process content"))
			}
			keys = &processedKeys
	...
	if err != nil {
		return errorwrap.WrapError("Cannot update evidence", errorwrap.DatabaseErr(err))
	}
...
func MoveEvidence(ctx context.Context, db *database.Connection, i MoveEvidenceInput) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to move evidence (src)", errorwrap.UnauthorizedReadErr(err))
	}

	destinationOperation, err := lookupOperation(db, i.TargetOperationSlug)
	if err != nil {
		return errorwrap.WrapError("Unable to move evidence (dst op)", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err := policyRequireWithAdminBypass(ctx,
		...
		return errorwrap.WrapError("Unwilling to move evidence", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Unable to list tag differences for moving", err)
	}
	...
	if err != nil {
		return errorwrap.WrapError("Cannot move evidence", err)
	}
