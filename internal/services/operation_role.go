...
func SetUserOperationRole(ctx context.Context, db *database.Connection, i SetUserOperationRoleInput) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to set user role", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Unable to get user id from slug", errorwrap.BadInputErr(err, fmt.Sprintf(`No user with slug "%s" was found`, i.UserSlug)))
	}

	if err := policyRequireWithAdminBypass(ctx, policy.CanModifyUserOfOperation{UserID: userID, OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to set user role", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if i.Role == "" {
		...
		if err != nil {
			return errorwrap.WrapError("Cannot delete user role", errorwrap.DatabaseErr(err))
		}
		return nil
	}
	...
	if err != nil {
		...
		if err != nil {
			return errorwrap.WrapError("Unable to add user role", errorwrap.DatabaseErr(err))
		}
		return nil
	}
	...
	if permission.Role != i.Role {
		...
		if err != nil {
			return errorwrap.WrapError("Unable to alter user role", errorwrap.DatabaseErr(err))
		}
	}
	return nil
}
...
func SetUserGroupOperationRole(ctx context.Context, db *database.Connection, i SetUserGroupOperationRoleInput) error {
	operation, err := lookupOperation(db, i.OperationSlug)
	if err != nil {
		return errorwrap.WrapError("Unable to set user group role", errorwrap.UnauthorizedWriteErr(err))
	}

	if i.UserGroupSlug == "" {
		return errorwrap.MissingValueErr("User Group Slug")
	}

	userGroupID, err := userGroupSlugToUserGroupID(db, i.UserGroupSlug)
	if err != nil {
		return errorwrap.WrapError("Unable to get user group id from slug", errorwrap.BadInputErr(err, fmt.Sprintf(`No user with slug "%s" was found`, i.UserGroupSlug)))
	}
	...
	if err := policyRequireWithAdminBypass(ctx, policy.CanModifyUserGroupOfOperation{UserGroupID: userGroupID, OperationID: operation.ID}); err != nil {
		return errorwrap.WrapError("Unwilling to set user group role", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if i.Role == "" {
		...
		if err != nil {
			return errorwrap.WrapError("Cannot delete user group role", errorwrap.DatabaseErr(err))
		}
		return nil
	}
