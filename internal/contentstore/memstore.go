...
func (d *MemStore) Upload(data io.Reader) (key string, err error) {
	...
	if err != nil {
		err = errorwrap.WrapError("Unable to add to MemStore", err)
	}
...
func (d *MemStore) UploadWithName(key string, data io.Reader) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable upload with a given name to MemStore", err)
	}
...
func (d *MemStore) Read(key string) (io.Reader, error) {
	...
	d.mutex.Unlock()
	if !ok {
		return nil, errorwrap.WrapError("Unable to read from MemStore", fmt.Errorf("No such key"))
	}
...
func (d *MemStore) Delete(key string) error {
	...
	if _, ok := d.content[key]; !ok { // artificial behavior to match other stores
		return errorwrap.WrapError("Unable to delete from MemStore", fmt.Errorf("No such key"))
	}
