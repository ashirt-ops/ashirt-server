...
func NewGCPStore(bucketName string) (*GCPStore, error) {
	...
	if err != nil {
		return nil, errors.WrapError("Unable to establish an gcp session", err)
	}
	return &GCPStore{
...
func (s *GCPStore) UploadWithName(key string, data io.Reader) error {
	...
	if _, err := io.Copy(wc, data); err != nil {
		return errors.WrapError("Upload to gcp failed", err)
	}

	if err := wc.Close(); err != nil {
		return errors.WrapError("Unable to close gcp writer", err)
	}
...
func (s *GCPStore) Read(key string) (io.Reader, error) {
	...
	if err != nil {
		return nil, errors.WrapError("Unable to read from gcp", err)
	}
	return res, nil
}
...
func (s *GCPStore) Delete(key string) error {
	...
	if err != nil {
		return errors.WrapError("Delete from gcp failed", err)
	}
