...
func NewDevStore() (*DevStore, error) {
	...
	if err != nil || !tmpDirInfo.IsDir() {
		...
		if err != nil {
			return nil, errorwrap.WrapError("Unable to establish a DevStore", err)
		}
	}
...
func (d *DevStore) Upload(data io.Reader) (string, error) {
	...
	if err != nil {
		return "", errorwrap.WrapError("Unable to upload to DevStore", err)
	}
	defer file.Close()
	_, err = bufio.NewReader(data).WriteTo(file)
	if err != nil {
		return "", errorwrap.WrapError("Unable upload to DevStore", err)
	}
...
func (d *DevStore) Read(key string) (io.Reader, error) {
	...
	if err != nil {
		return reader, errorwrap.WrapError("Unable to read file from DevStore", err)
	}
	return reader, err
}
...
func (d *DevStore) Delete(key string) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to delete file from DevStore", err)
	}
	return err
}
