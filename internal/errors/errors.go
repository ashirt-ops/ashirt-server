...
var (
	...
	ErrorDeprecated error = stderrors.New("warning: deprecated")
)
...
func MissingValueErr(valueName string) error {
	...
	return HTTPErr(http.StatusBadRequest, reason, stderrors.New(reason))
}
...
func DisabledUserError() error {
	//lint:ignore ST1005 Returned directly to the frontend for render
	return stderrors.New("This account has been disabled. Please contact an adminstrator if you think this is an error.")
}

// PanicedError represents any error the occurs
func PanicedError() error {
	return HTTPErr(http.StatusInternalServerError, "An unknown error occurred", stderrors.New("pancied during processing"))
}
