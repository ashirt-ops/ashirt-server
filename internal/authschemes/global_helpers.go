...
func CreateNewAuthForUserGeneric(db *database.Connection, authSchemeName, authSchemeType string, data UserAuthData) error {
	...
	if err != nil {
		if database.IsAlreadyExistsError(err) {
			return errorwrap.BadInputErr(err, "An account for this user already exists")
		}
		return errorwrap.WrapError("Unable to generate auth scheme for user", errorwrap.DatabaseErr(err))
	}
	return nil
}
