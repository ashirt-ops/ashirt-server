...
func (o OIDCAuth) handleCallback(w http.ResponseWriter, r *http.Request, bridge authschemes.AShirtAuthBridge) (interface{}, error) {
	...
	if !ok {
		return o.authFailure(w, r, errors.BadAuthErr(stderrors.New(authName+" callback called without preloginauth session")), "/autherror/noaccess")
	}
	...
	if r.URL.Query().Get("state") != sess.StateChallengeCSRF || oidcExchangeCode == "" {
		return o.authFailure(w, r, errors.BadAuthErr(stderrors.New(authName+" authentication challenge failed")), "/autherror/noverify")
	}

	oauth2Token, err := o.oauthConfig.Exchange(r.Context(), r.URL.Query().Get("code"))

	if err != nil {
		return o.authFailure(w, r, errors.BadAuthErr(err), "/autherror/noverify")
	}

	rawIDToken, ok := oauth2Token.Extra("id_token").(string)
	if !ok {
		return o.authFailure(w, r, errors.BadAuthErr(stderrors.New(authName+" no id_token field in oauth2 token")), "/autherror/noverify")
	}
	idToken, err := o.verifier.Verify(r.Context(), rawIDToken)
	if err != nil {
		return o.authFailure(w, r, errors.BadAuthErr(fmt.Errorf("%s authentication token verification failed: %w", authName, err)), "/autherror/noverify")
	}

	tokenSource := o.oauthConfig.TokenSource(r.Context(), oauth2Token)
	profile, err := o.provider.UserInfo(r.Context(), tokenSource)
	if err != nil {
		return o.authFailure(w, r, errors.BadAuthErr(stderrors.New(authName+" user is not permitted access")), "/autherror/noaccess")
	}

	profileClaims := make(map[string]interface{})
	if err = profile.Claims(&profileClaims); err != nil {
		return o.authFailure(w, r, errors.BadAuthErr(stderrors.New(authName+" unable to parse profile claims")), "/autherror/noaccess")
	}

	userProfile, err := o.makeUserProfile(profileClaims)
	if err != nil {
		return o.authFailure(w, r, errors.WrapError("Unable to read claim data", err), "/autherror/incomplete")
	}
	...
	if err != nil { //an error here implies that a user doesn't yet exist
		...
		if linkingAccount {
			...
			if !o.registrationEnabled {
				return o.authFailure(w, r, errors.WrapError("Registration is disabled", err), "/autherror/registrationdisabled")
			}

			userResult, err := bridge.CreateNewUser(*userProfile)
			if err != nil {
				return o.authFailure(w, r, errors.WrapError("Create new "+authName+" user failed ["+userProfile.Slug+"]", err), "/autherror/incomplete")
			}
		...
		if err != nil {
			return o.authFailure(w, r, errors.WrapError("Unable to create auth scheme for new "+authName+" user ["+authData.Username+"]", err), "/autherror/incomplete")
		}
	}
	if linkingAccount {
	...
	if err != nil {
		if errors.IsErrorAccountDisabled(err) {
			return o.authFailure(w, r, errors.WrapError("Unable to log in "+authName+" user ["+authData.Username+"]", err), "/autherror/disabled")
		}
		return o.authFailure(w, r, errors.WrapError("Unable to log in "+authName+" user ["+authData.Username+"]", err), "/autherror/incomplete")
	}
	return o.authSuccess(w, r, linkingAccount)
}
