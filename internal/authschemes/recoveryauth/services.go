...
func DeleteExpiredRecoveryCodes(ctx context.Context, db *database.Connection, expiryInMinutes int64) error {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return errors.WrapError("Insufficient access to remove recovery code", errors.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errors.WrapError("Unable to remove recovery code", errors.DatabaseErr(err))
	}
...
func generateRecoveryCodeForUser(ctx context.Context, bridge authschemes.AShirtAuthBridge, userSlug string) (interface{}, error) {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return nil, errors.WrapError("Non-admin tried to generate recovery code", errors.UnauthorizedWriteErr(err))
	}

	userID, err := bridge.GetUserIDFromSlug(userSlug)
	if err != nil {
		return nil, errors.WrapError("Unable to get UserID from slug", err)
	}

	authKeyStr, err := recoveryHelpers.GenerateRecoveryCodeForUser(bridge.GetDatabase(), userID)
	if err != nil {
		return nil, errors.WrapError("Could not generate recovery code for user", err)
	}
	...
	if err != nil {
		return response, errors.WrapError("Unable to create recovery auth for user", err)
	}
	return response, nil
}
...
func getRecoveryMetrics(ctx context.Context, db *database.Connection, expiryInMinutes int64) (interface{}, error) {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return nil, errors.WrapError("Non-admin tried to get recovery metrics", errors.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return metrics, errors.WrapError("Failed to query recovery metrics", err)
	}
...
func generateRecoveryEmail(ctx context.Context, bridge authschemes.AShirtAuthBridge, userEmail string) error {
	...
	if err != nil {
		return errors.WrapError("Unable to get user account from email", err)
	}
	...
	if err != nil {
		return errors.WrapError("Unable to generate recovery email", err)
	}
