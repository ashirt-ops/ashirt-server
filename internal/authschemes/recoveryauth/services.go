...
import (
	...
	"github.com/ashirt-ops/ashirt-server/internal/emailtemplates"
...
func DeleteExpiredRecoveryCodes(ctx context.Context, db *database.Connection, expiryInMinutes int64) error {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return errorwrap.WrapError("Insufficient access to remove recovery code", errorwrap.UnauthorizedWriteErr(err))
	}
	...
	if err != nil {
		return errorwrap.WrapError("Unable to remove recovery code", errorwrap.DatabaseErr(err))
	}
...
func generateRecoveryCodeForUser(ctx context.Context, bridge authschemes.AShirtAuthBridge, userSlug string) (interface{}, error) {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return nil, errorwrap.WrapError("Non-admin tried to generate recovery code", errorwrap.UnauthorizedWriteErr(err))
	}

	userID, err := bridge.GetUserIDFromSlug(userSlug)
	if err != nil {
		return nil, errorwrap.WrapError("Unable to get UserID from slug", err)
	}

	authKeyStr, err := recoveryHelpers.GenerateRecoveryCodeForUser(bridge.GetDatabase(), userID)
	if err != nil {
		return nil, errorwrap.WrapError("Could not generate recovery code for user", err)
	}
	...
	if err != nil {
		return response, errorwrap.WrapError("Unable to create recovery auth for user", err)
	}
	return response, nil
}
...
func getRecoveryMetrics(ctx context.Context, db *database.Connection, expiryInMinutes int64) (interface{}, error) {
	if err := policy.Require(middleware.Policy(ctx), policy.AdminUsersOnly{}); err != nil {
		return nil, errorwrap.WrapError("Non-admin tried to get recovery metrics", errorwrap.UnauthorizedReadErr(err))
	}
	...
	if err != nil {
		return metrics, errorwrap.WrapError("Failed to query recovery metrics", err)
	}
...
func generateRecoveryEmail(ctx context.Context, bridge authschemes.AShirtAuthBridge, userEmail string) error {
	...
	if err != nil {
		return errorwrap.WrapError("Unable to get user account from email", err)
	}
	...
	if err != nil {
		return errorwrap.WrapError("Unable to generate recovery email", err)
	}
