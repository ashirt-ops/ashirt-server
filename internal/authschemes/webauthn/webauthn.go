...
func (a WebAuthn) BindRoutes(r chi.Router, bridge authschemes.AShirtAuthBridge) {
	...
	remux.Route(r, "POST", "/register/finish", remux.JSONHandler(func(r *http.Request) (interface{}, error) {
		...
		if err != nil {
			return nil, errorwrap.WrapError("Unable to validate registration data", err)
		}
		...
		if err != nil {
			return nil, errorwrap.WrapError("Unable to create user", err)
		}
	...
	remux.Route(r, "POST", "/login/finish", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		remux.JSONHandler(func(r *http.Request) (interface{}, error) {
			...
			if discoverable {
				...
				if err != nil {
					return nil, errorwrap.WrapError("error parsing credential", errorwrap.BadAuthErr(err))
				}
				...
				userHandler := func(_, userHandle []byte) (user webauthn.User, err error) {
					...
					if err != nil {
						return nil, errorwrap.WebauthnLoginError(err, "Could not find user from authn ID", "No such user found")
					}
					auth, err := bridge.FindUserAuthByUserID(dbUser.ID)
					if err != nil {
						return nil, errorwrap.DatabaseErr(err)
					}
				...
				if err != nil {
					return nil, errorwrap.BadAuthErr(err)
				} else if cred.Authenticator.CloneWarning {
					return nil, errorwrap.WrapError("credential appears to be cloned", errorwrap.BadAuthErr(err))
				}

				err = bridge.SetAuthSchemeSession(w, r, makeWebauthNSessionData(webauthnUser, data.WebAuthNSessionData))
				if err != nil {
					return nil, errorwrap.WebauthnLoginError(err, "Unable to finish login process", "Unable to set session")
				}
				rawData = bridge.ReadAuthSchemeSession(r)
				data, ok = rawData.(*webAuthNSessionData)
				if !ok {
			...
			if err := bridge.LoginUser(w, r, data.UserData.UserIDAsI64(), nil); err != nil {
				return nil, errorwrap.WrapError("Attempt to finish login failed", err)
			}
	...
	remux.Route(r, "POST", "/credential/add/finish", remux.JSONHandler(func(r *http.Request) (interface{}, error) {
		...
		if err != nil {
			return nil, errorwrap.WrapError("Unable to find user", err)
		}
		userAuth.JSONData = helpers.Ptr(string(encodedCreds))
		err = bridge.UpdateAuthForUser(userAuth)
		if err != nil {
			return nil, errorwrap.WrapError("Unable to update credentials", err)
		}
...
func (a WebAuthn) getCredentials(userID int64, bridge authschemes.AShirtAuthBridge) (*ListCredentialsOutput, error) {
	...
	if err != nil {
		return nil, errorwrap.WrapError("Unable to get credentials", err)
	}

	webauthRawCreds := []byte(*auth.JSONData)
	var creds []AShirtWebauthnCredential
	if err = json.Unmarshal(webauthRawCreds, &creds); err != nil {
		return nil, errorwrap.WebauthnLoginError(err, "Unable to parse webauthn credentials")
	}
...
func (a WebAuthn) deleteCredential(userID int64, credentialID []byte, bridge authschemes.AShirtAuthBridge) error {
	auth, err := bridge.FindUserAuthByUserID(userID)
	if err != nil {
		return errorwrap.WrapError("Unable to find user", err)
	}

	webauthRawCreds := []byte(*auth.JSONData)
	var creds []AShirtWebauthnCredential
	if err = json.Unmarshal(webauthRawCreds, &creds); err != nil {
		return errorwrap.WebauthnLoginError(err, "Unable to parse webauthn credentials")
	}
	...
	if err != nil {
		return errorwrap.WrapError("Unable to delete credential", err)
	}
...
func (a WebAuthn) updateCredentialName(info WebAuthnUpdateCredentialInfo, bridge authschemes.AShirtAuthBridge) error {
	...
	if matchingIndex == -1 {
		return errorwrap.WrapError("Could not find matching credential", err)
	}
	...
	if err != nil {
		return errorwrap.WrapError("Unable to encode credentials", err)
	}
	userAuth.JSONData = helpers.Ptr(string(encodedCreds))
	if err = bridge.UpdateAuthForUser(userAuth); err != nil {
		return errorwrap.WrapError("Unable to update credential", err)
	}
	return nil
}
...
func (a WebAuthn) beginRegistration(w http.ResponseWriter, r *http.Request, bridge authschemes.AShirtAuthBridge, info WebAuthnRegistrationInfo) (*protocol.CredentialCreation, error) {
	...
	if idx != -1 {
		return nil, errorwrap.BadInputErr(
...
func (a WebAuthn) beginLogin(w http.ResponseWriter, r *http.Request, bridge authschemes.AShirtAuthBridge, username string) (interface{}, error) {
	...
	if discoverable {
		...
		if err != nil {
			return nil, errorwrap.WebauthnLoginError(err, "Unable to find login credentials", "Unable to find login credentials")
		}
		data = makeDiscoverableWebauthNSessionData(sessionData)
	} else {
		authData, err := bridge.FindUserAuth(username)
		if err != nil {
			return nil, errorwrap.WebauthnLoginError(err, "Could not validate user", "No such auth")
		}
		if authData.JSONData == nil {
			return nil, errorwrap.WebauthnLoginError(err, "User lacks webauthn credentials")
		}

		user, err := bridge.GetUserFromID(authData.UserID)
		if err != nil {
			return nil, errorwrap.WebauthnLoginError(err, "Could not validate user", "No such user")
		}
		...
		if err != nil {
			return nil, errorwrap.WebauthnLoginError(err, "Unable to begin login process")
		}
	...
	if err != nil {
		return nil, errorwrap.WebauthnLoginError(err, "Unable to begin login process", "Unable to set session")
	}
...
func (a WebAuthn) validateRegistrationComplete(r *http.Request, bridge authschemes.AShirtAuthBridge) (*webAuthNSessionData, []byte, error) {
	...
	if err != nil {
		return nil, nil, errorwrap.WrapError("Unable to complete registration", err)
	}
	...
	if err != nil {
		return nil, nil, errorwrap.WrapError("Unable to create registration", err)
	}
