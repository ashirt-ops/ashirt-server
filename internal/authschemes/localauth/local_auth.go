...
import (
	...
	"github.com/ashirt-ops/ashirt-server/internal/authschemes/localauth/constants"
...
func (p LocalAuthScheme) BindRoutes(r chi.Router, bridge authschemes.AShirtAuthBridge) {
	...
	remux.Route(r, "POST", "/admin/register", remux.JSONHandler(func(r *http.Request) (interface{}, error) {
		if !middleware.IsAdmin(r.Context()) {
			return nil, errors.UnauthorizedWriteErr(fmt.Errorf("Requesting user is not an admin"))
		}

		authKey := make([]byte, 42)
		if _, err := rand.Read(authKey); err != nil {
			return nil, errors.WrapError("Unable to generate random new user password key", err)
		}
	...
	remux.Route(r, "POST", "/login", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		remux.JSONHandler(func(r *http.Request) (interface{}, error) {
			...
			if firstErr := errors.FirstError(findUserErr, checkPwErr); firstErr != nil {
				return nil, errors.WrapError("Could not validate user", errors.InvalidCredentialsErr(firstErr))
			}
	...
	remux.Route(r, "POST", "/login/resetpassword", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		remux.JSONHandler(func(r *http.Request) (interface{}, error) {
			...
			if !sess.SessionValid {
				return nil, errors.HTTPErr(http.StatusUnauthorized,
					"Your account does not require a password reset at this time",
					stderrors.New("User session is not a local auth needsPasswordResetAuthSession"))
			}

			if err := updateUserPassword(bridge, sess.Username, newPassword); err != nil {
				return nil, errors.WrapError("Unable to reset user password", err)
			}
		...
		remux.JSONHandler(func(r *http.Request) (interface{}, error) {
			...
			if !sess.SessionValid {
				return nil, errors.HTTPErr(http.StatusUnauthorized,
					...
					stderrors.New("User session does not require needsTotpAuthSession"))
			}

			authData, err := bridge.FindUserAuth(sess.Username)
			if err != nil {
				return nil, errors.WrapError("Could not validate passcode", err)
			}

			if authData.TOTPSecret == nil {
				return nil, errors.HTTPErr(http.StatusUnauthorized,
					"Could not validate passcode",
					stderrors.New("User trying to authenticate with TOTP when TOTP is not enabled"))
			}
			...
			if err = sess.writeLocalSession(w, r, bridge); err != nil {
				return nil, errors.WrapError("Could not validate passcode", errors.WrapError("Unable to set auth scheme in session", err))
			}
	...
	remux.Route(r, "PUT", "/password", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		remux.JSONHandler(func(r *http.Request) (interface{}, error) {
			...
			if firstErr := errors.FirstError(findUserErr, checkPwErr); firstErr != nil {
				return nil, errors.WrapError("Unable to set new password", errors.InvalidPasswordErr(firstErr))
			}
			if authData.UserID != middleware.UserID(r.Context()) {
				return nil, errors.InvalidPasswordErr(stderrors.New("Cannot reset password for a different user than is currently logged in"))
			}
	...
	remux.Route(r, "PUT", "/admin/password", remux.JSONHandler(func(r *http.Request) (interface{}, error) {
		...
		if err != nil {
			return nil, errors.NotFoundErr(fmt.Errorf("User %v does not have %v authentication", userSlug, p.Name()))
		}
		...
		if err != nil {
			return nil, errors.WrapError("Unable to encrypt new password", err)
		}
	...
	remux.Route(r, "POST", "/totp", remux.JSONHandler(func(r *http.Request) (interface{}, error) {
		...
		if userAuth.TOTPSecret != nil {
			return nil, errors.BadInputErr(
				stderrors.New("User already has a TOTP key associated"),
...
func attemptFinishLogin(w http.ResponseWriter, r *http.Request, bridge authschemes.AShirtAuthBridge, authData authschemes.UserAuthData) error {
	...
	if authData.TOTPSecret != nil {
		if !sess.SessionValid || !sess.TOTPValidated {
			...
			if err := sess.writeLocalSession(w, r, bridge); err != nil {
				return errors.WrapError("Unable to set auth scheme in session", err)
			}
			return errors.UserRequiresAdditionalAuthenticationErr("TOTP_REQUIRED")
		}
	}
	...
	if authData.NeedsPasswordReset {
		...
		return errors.UserRequiresAdditionalAuthenticationErr("PASSWORD_RESET_REQUIRED")
	}

	if err := bridge.LoginUser(w, r, authData.UserID, nil); err != nil {
		return errors.WrapError("Attempt to finish login failed", err)
	}
...
func updateUserPassword(bridge authschemes.AShirtAuthBridge, username string, newPassword string) error {
	...
	if err != nil {
		return errors.WrapError("Unable to update password", err)
	}
	...
	encryptedPassword, err := bcrypt.GenerateFromPassword([]byte(newPassword), bcrypt.DefaultCost)
	if err != nil {
		return errors.WrapError("Unable to encrypte new password", err)
	}
...
func checkPasswordComplexity(suggestedPassword string) error {
	err := stderrors.New("Password did not meet requirements")
	if len(suggestedPassword) < 5 {
		return errors.BadInputErr(err, "Password must be at least 5 characters long")
	}
	// TODO: Fill in with password complexity requirements/tests
	// if strings.Contains(suggestedPassword, "a") {
	//   return errors.BadInputErr(err, "Password must not use an `a` character")
	// }
	// if !strings.Contains(suggestedPassword, "0") {
	//   return errors.BadInputErr(err, "Password must contain a 0 character")
	// }
