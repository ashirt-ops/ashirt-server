...
func validateTOTP(passcode string, totpSecret string) error {
	if passcode == "" {
		return errorwrap.MissingValueErr("TOTP Passcode")
	}
	if totpSecret == "" {
		return errorwrap.MissingValueErr("TOTP Secret")
	}
	...
	if err != nil {
		return errorwrap.InvalidTOTPErr(err)
	}
	if !isValid {
		return errorwrap.InvalidTOTPErr(errors.New("totp.Validate failure"))
	}
	return nil
}
